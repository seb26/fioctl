...
DEFAULT_COLS = column_default(
    "teams", "id,account_id,name,creator_id,storage,member_count,collaborator_count"
)
...
@click.option(
    "--columns",
    type=utils.ListType(),
    default=["id", "user_id", "email", "inserted_at"],
)
def team_member(team_id, email, format, columns):
    team_member = fio_client()._api_call(
        "post", f"/teams/{team_id}/members", {"email": email, "role": "member"}
    )
